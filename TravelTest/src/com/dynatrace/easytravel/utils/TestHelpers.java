...
public class TestHelpers {
	...
	public static void runTestWithDifferentLogLevel(final Runnable test, final String className, final Level... levels) {
		...
		Logger localLogger = (Logger)org.slf4j.LoggerFactory.getLogger(className);//Logger.getLogger(className);
	...
	/**
	 * @return Returns true if running .NET procedures is possible on the current operating system and
	 * 				enabled in the current configuration/scenario.
	 * @author cwat-dstadler
	 */
	public static boolean isDotNetEnabled() {
    	// ensure that the Scenario is loaded here so we have the TechnologyActivator updated
    	ScenarioConfiguration conf = new ScenarioConfiguration();
    	conf.loadOrCreate();

    	// Windows and enabled in the scenario config
    	return OperatingSystem.WINDOWS.equals(OperatingSystem.pickUp()) &&
    			CentralTechnologyActivator.getIntance().isAllowed(Technology.DOTNET_20);
	}

	/**
	 * Wait for the specified port to be free again, fail if the port is still occupied after
	 * the specified number of milliseconds
	 *
	 * @param port
	 * @param milliseconds
	 */
	public static void waitForPort(int port, int milliseconds) {
		for(int i = 0;i < 20;i++) {
			if(SocketUtils.isPortAvailable(port, null) && SocketUtils.isPortAvailable(port, "localhost")) {
				// done already, no port is blocked any more
				return;
			}

			log.info("Waiting for port " + port + " to become available again...");
			try {
		...
		assertTrue("Port " + port + " is still occupied on default nic after " + milliseconds + "ms by process: " + getProcessOnPort(port),
				SocketUtils.isPortAvailable(port, null));
		assertTrue("Port " + port + " is still occupied on localhost after " + milliseconds + "ms by process: " + getProcessOnPort(port),
				SocketUtils.isPortAvailable(port, "localhost"));
	}

	public static String getProcessOnPort(int port) {
		NetstatUtil netstatUtil = new NetstatUtil(Runtime.getRuntime());
		return netstatUtil.findProcessForPort(port) + "(" + netstatUtil.findProcessIdForPort(port) + ")";
	}

	/**
	 * Check if we can successfully instantiate an SWT-Display to decide if a test can be executed or not
	 *
	 * @return
	 * @author cwat-dstadler
	 */
	public static void assumeCanUseDisplay() {
		boolean works = false;
		try {
			new Display().dispose();
			works = true;
	...
	/**
	 * Initialize plugin with name and extension point
	 * @param plugin
	 * @param name
	 * @param extensionPoint
	 */
	public static void initPlugin(AbstractGenericPlugin plugin, String name, String[] extensionPoint) {
		plugin.setName(name);
	 ...
	 * @param enabled
	 */
	public static void enablePlugin(String name, boolean enabled) {
		SpringUtils.getPluginStateProxy().setPluginEnabled(name, enabled);
	}

	/**
	 * Check if plugin with given name is enabled
	 * @param name
	 * @return
	 */
	public static boolean isPluginEnabled(String name) {
		String[] enabledPluginNames = SpringUtils.getPluginStateProxy().getEnabledPluginNames();
